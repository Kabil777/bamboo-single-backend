...
import { JWTPayload, jwtVerify, SignJWT, importSPKI, importPKCS8, exportJWK } from "jose";
...
export class JwtHelper {
    public async verifyAccessToken(token: string): Promise<JWTPayload> {
        if (!publicKeyObj) publicKeyObj = await importSPKI(publicKeyPem, "RS256");
    ...
    public async signAccessToken(payload: JWTPayload, expiresIn: string | number): Promise<string> {
        if (!privateKeyObj) privateKeyObj = await importPKCS8(privateKeyPem, "RS256");
