...
const html = (title: string, body: string) => `<h1>${title}</h1><p>${body}</p>`;

async function seed(spec: ContentSpec) {
    ...
    if (!owner) throw new Error("Create a user before seeding content");
