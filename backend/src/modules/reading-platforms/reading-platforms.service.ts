...
export class ReadingPlatformsService {
    ...
    async create(ownerId: string, input: unknown) {
        ...
        if (existing) throw new ValidationError("A platform with this name already exists");
    ...
    async remove(id: string) {
        ...
        if (!platform) throw new NotFoundError("Reading platform not found");
    ...
    private validateInput(input: unknown) {
        ...
        const { name, websiteUrl, coverSourceUrl, description } = input as Record<string, unknown>;
        if (typeof name !== "string" || !name.trim() || name.trim().length > 100) throw new ValidationError("name must be a non-empty string up to 100 characters");
        if (typeof websiteUrl !== "string" || !this.isHttpUrl(websiteUrl)) throw new ValidationError("websiteUrl must be a valid http or https URL");
        if (typeof coverSourceUrl !== "string" || !this.isHttpUrl(coverSourceUrl)) throw new ValidationError("coverSourceUrl must be a valid http or https image URL");
        if (description !== undefined && description !== null && (typeof description !== "string" || description.length > 1000)) throw new ValidationError("description must be at most 1000 characters");
    ...
    private toSlug(name: string) {
        ...
        if (!slug || slug.length > 50) throw new ValidationError("name must produce a URL-safe slug up to 50 characters");
        return slug;
    }
}
