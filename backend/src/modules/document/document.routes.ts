...
router.get("/me", authRequired, async (req, res, next) => {
    try {
        const data = await prisma.document.findMany({
            where: { authorId: req.headers["x-user-id"] as string },
...
router.post("/", authRequired, async (req, res, next) => {
    try { res.status(201).json(await documentService.create(req.headers["x-user-id"] as string, req.body)); } catch (error) { next(error); }
});
...
router.get("/:id", authOptional, async (req, res, next) => {
    try {
        ...
        if (document.visibility === "PRIVATE" && !(await canManage(document.authorId, req.headers["x-user-id"] as string | undefined))) throw new NotFoundError("Document not found");
        res.json({ ...document, viewerCanEdit: await canManage(document.authorId, req.headers["x-user-id"] as string | undefined) });
    } catch (error) { next(error); }
});
...
router.patch("/:id/pages/:pageId", authRequired, async (req, res, next) => {
    try { res.json(await documentService.updatePage(String(req.params.id), String(req.params.pageId), req.headers["x-user-id"] as string, req.body)); } catch (error) { next(error); }
});

export default router;

async function canManage(authorId: string, actorId?: string) {
