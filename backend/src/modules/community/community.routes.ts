...
router.get("/users/:id", authOptional, async (req, res, next) => {
    try {
        ...
        const isOwner = await canManage(user.id, req.headers["x-user-id"] as string | undefined);
...
router.get("/users/:id/posts", authOptional, async (req, res, next) => {
    try {
        ...
        const isOwner = await canManage(authorId, req.headers["x-user-id"] as string | undefined);
...
function countByVisibility(rows: Array<{ visibility: string; _count: { _all: number } }>) {
