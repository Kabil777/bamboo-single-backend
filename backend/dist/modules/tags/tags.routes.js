...
router.get("/", async (_req, res, next) => {
    try {
        res.json({ data: await prisma.tag.findMany({ orderBy: [{ category: "asc" }, { label: "asc" }] }) });
    }
    catch (error) {
        next(error);
    }
...
router.post("/", authRequired, adminRequired, async (req, res, next) => {
    try {
        ...
        if (typeof label !== "string" || !label.trim() || label.trim().length > 80)
            throw new ValidationError("label must be a non-empty string up to 80 characters");
        if (typeof category !== "string" || !categories.has(category))
            throw new ValidationError("category must be DESIGNATION or INTEREST");
        const normalized = label.trim();
        const exists = await prisma.tag.findFirst({ where: { category, label: { equals: normalized, mode: "insensitive" } } });
        if (exists)
            throw new ValidationError("This tag already exists in that category");
