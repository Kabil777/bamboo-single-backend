...
const authorSelect = { id: true, name: true, pictureUrl: true };
function toPostResponse(post, viewerHasLiked = false, viewerHasBookmarked = false, viewerCanEdit = false) {
    return {
        id: post.id,
        title: post.title,
...
export class PostsService {
    ...
    async getById(id, viewerId) {
        ...
        if (post.visibility === "PRIVATE" && !viewerCanEdit)
            throw new NotFoundError("Post not found");
    ...
    async like(id, userId, shouldLike) {
    ...
    async bookmark(id, userId, shouldBookmark) {
        const post = await prisma.post.findUnique({ where: { id }, select: { id: true } });
        if (!post)
            throw new NotFoundError("Post not found");
        if (shouldBookmark) {
    ...
    validatePostInput(input, partial) {
